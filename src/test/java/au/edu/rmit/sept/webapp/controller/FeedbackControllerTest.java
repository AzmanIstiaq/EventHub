...
class FeedbackControllerTest {
    ...
    @DisplayName("US: Student submits feedback -> redirects")
    void submitFeedbackRedirects() throws Exception {
        ...
        when(userService.findById(studentId)).thenReturn(Optional.of(u));

        mvc.perform(post("/events/feedback/{eventId}", eventId)
                        .param("rating", "4")
                        .param("comment", "Nice")
                        .with(csrf()))
