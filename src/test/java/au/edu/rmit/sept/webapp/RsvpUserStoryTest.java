...
class RsvpUserStoryTest {
    ...
    EventController eventController = new EventController(eventService, registrationService, userService, categoryService);
    ...
    void eventListLoads() throws Exception {
        Event e = new Event(); e.setEventId(10L); e.setTitle("Welcome Week"); e.setDateTime(LocalDateTime.now().plusDays(2));
        ...
        String view = eventController.listEvents(null, model);
    ...
    void rsvpRegistersAndRedirects() {
        ...
        Event e = new Event(); e.setEventId(eventId); e.setTitle("Welcome Week"); e.setDateTime(LocalDateTime.now().plusDays(3));
    ...
    void myEventsListsRsvpedEvent() {
        ...
        when(registrationRepository.findByUser_UserId(userId)).thenReturn(List.of(r));
    ...
    void userNotFoundRsvp() {
        ...
        Event e = new Event(); e.setEventId(eventId); e.setTitle("Welcome Week"); e.setDateTime(LocalDateTime.now().plusDays(2));
        when(eventService.findById(eventId)).thenReturn(Optional.of(e));
    ...
    void emptySearchOk() {
        ...
        Model model = new ExtendedModelMap();
        String view = eventController.searchEvents(null, "", null, null, null, model);
        assertThat(view).isEqualTo("public-events");
    ...
    void dateRangeEndBeforeStartOk() {
        ...
        Model model = new ExtendedModelMap();
        String view = eventController.searchEvents(null, "q", LocalDate.now().plusDays(10), LocalDate.now().plusDays(5), null, model);
        assertThat(view).isEqualTo("public-events");
        assertThat(model.containsAttribute("searchResults")).isTrue();
    ...
    void onlyEndDateProvidedOk() {
        ...
        Model model = new ExtendedModelMap();
        String view = eventController.searchEvents(null, "q", null, LocalDate.now().plusDays(1), null, model);
        assertThat(view).isEqualTo("public-events");
        assertThat(model.containsAttribute("searchResults")).isTrue();
        verify(eventService).searchEvents(any(), any(), any(), any());
    }
